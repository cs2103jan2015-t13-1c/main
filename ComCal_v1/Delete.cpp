...
std::string Delete::executeInput(std::string userInput) {
	...
	for (unsigned int i = 0; i < _deleteTaskIndexes.size(); i++) {
		if (contents[_deleteTaskIndexes[i]].getCode() == CODE_INVALID) {
...
std::string Delete::undo(void) {
	...
	for (unsigned int i = 0; i < deletedTasks.size(); i++) {
		...
		for (unsigned int j = 0; j < contents.size() && !isFound; j++) {
			if (contents[j].getCode() == CODE_INVALID && contents[j].getDescription() == deletedTasks[i].getDescription() && contents[j].getStartDateTime() == deletedTasks[i].getStartDateTime() && contents[j].getEndDateTime() == deletedTasks[i].getEndDateTime() && contents[j].getLocation() == deletedTasks[i].getLocation()) {

...
//processTime keyword type strings and index int:
const int SIZE_TYPEKEYWORDSARRAY = 3 + 7 + 1;
const std::string TYPEKEYWORDS[SIZE_TYPEKEYWORDSARRAY] = { "today", "tomorrow", "yesterday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday", "week" };
const int INDEX_TODAY = 0;
const int INDEX_TOMORROW = 1;
const int INDEX_YESTERDAY = 2;
const int INDEX_MONDAY = 3;
const int INDEX_TUESDAY = 4;
const int INDEX_WEDNESDAY = 5;
const int INDEX_THURSDAY = 6;
const int INDEX_FRIDAY = 7;
const int INDEX_SATURDAY = 8;
const int INDEX_SUNDAY = 9;
const int INDEX_WEEK = 10;

//this/next/last strings and index int declarations:
const int SIZE_THISNEXTLASTARRAY = 3;
const std::string THISNEXTLAST[SIZE_THISNEXTLASTARRAY] = { "this", "next", "last" };
const int INDEX_THIS = 0;
const int INDEX_NEXT = 1;
const int INDEX_LAST = 2;

//DD, MM, YY, HH and MM length int:
...
//DD/MM/YY date input seperator string and position int:
const int SIZE_SEPARATORSARRAY = 1;
const char SEPARATORS[SIZE_SEPARATORSARRAY] = { '/' };
const int POSITION_SEPARATOR1 = LENGTH_MDAYINPUT;
const int POSITION_SEPARATOR2 = LENGTH_MDAYINPUT + 1 + LENGTH_MONTHINPUT;

//Time in seconds int:
const int ONEDAY = 24 * 60 * 60;
const int ONEWEEK = ONEDAY * 7;

//Undefined time code int:
...
const int LENGTH_DATESTRING = 16;
const int LENGTH_TIMESTRING = 7;

//Attribute indentation position int for wrap text:
const int POS_ATTRIBUTE_LINE = 17;
			...
			if (inputs[i] == TYPEKEYWORDS[j]) {
				if (j >= INDEX_TODAY && j <= INDEX_YESTERDAY) {
					...
					if (inputs.empty()) {
						...
						if (inputs[0].find_first_not_of(INTS) != std::string::npos) {
							throw exceptionInputInvalidTime;
						}
						...
						for (int k = 0; (k < SIZE_SEPARATORSARRAY) && (!processed); k++) {
							if (inputs[0][POSITION_SEPARATOR1] == SEPARATORS[k] && inputs[0][POSITION_SEPARATOR2] == SEPARATORS[k]) {
								if (inputs[0].size() == LENGTH_MDAYINPUT + 1 + LENGTH_MONTHINPUT + 1 + LENGTH_YEARINPUT) {
									if (inputs[0].substr(0, LENGTH_MDAYINPUT).find_first_not_of(INTS) != std::string::npos) {
										throw exceptionInputInvalidDate;
									}

									if (inputs[0].substr(POSITION_SEPARATOR1 + 1, LENGTH_MONTHINPUT).find_first_not_of(INTS) != std::string::npos) {
										throw exceptionInputInvalidDate;
									}

									if (inputs[0].substr(POSITION_SEPARATOR2 + 1, LENGTH_YEARINPUT).find_first_not_of(INTS) != std::string::npos) {
										throw exceptionInputInvalidDate;
									}
...
time_t Command::getYesterdayTime(bool isStart, int hour, int min){
	...
	time_t timeDesired = timeNow - ONEDAY;
...
time_t Command::getDayTime(bool isStart, int day, int thisNextLast, int hour, int min) {
	...
	desiredDayOfWeekTimeT = timeNow + (daysDifference * ONEDAY);

	time_t timeDesired;

	if (daysDifference < 0) {
		desiredDayOfWeekTimeT += ONEWEEK;
	}

	if (thisNextLast == INDEX_NEXT) {
		timeDesired = desiredDayOfWeekTimeT + ONEWEEK;
	}
...
time_t Command::getWeekTime(int thisNextLast, int mday, int month, int year) {
	...
	if (mday != UNDEFINED && month != UNDEFINED && year != UNDEFINED) {
		...
		startOfWeekTimeT = timeDesired + (daysDifference * ONEDAY);
	}
	else {
		int wdayToday;
		wdayToday = structTimeT.tm_wday - 1;
		...
		startOfWeekTimeT = timeNow + (daysDifference * ONEDAY);

		if (thisNextLast == INDEX_NEXT) {
			startOfWeekTimeT += ONEWEEK;
		}
		else if (thisNextLast == INDEX_LAST) {
			startOfWeekTimeT -= ONEWEEK;
		}
...
std::string Command::prepareTaskDisplayOneLine(Task task) {
	...
	if (task.getStartDateTime() > 0) {
		...
		if (task.getEndDateTime() > 0) {
			...
			if (startDateTimeString.substr(0, LENGTH_DATESTRING) == endDateTimeString.substr(0, LENGTH_DATESTRING)) {
				endDateTimeString = endDateTimeString.substr(endDateTimeString.size() - LENGTH_TIMESTRING);
			}
...
std::string Command::getTaskType(std::string typeCode) {
	...
	if (typeCode == CODE_TIMED || typeCode == CODE_PARTIALTIMED) {
		editedType = TYPE_EVENT;
	}
	else if (typeCode == CODE_DEADLINE) {
		editedType = TYPE_DEADLINE;
	}
	else {
		editedType = TYPE_TASK;
	}

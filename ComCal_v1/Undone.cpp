...
std::string Undone::executeInput(std::string inputString) {
	...
	for (unsigned int i = 0; i < _undoneTaskIndexes.size(); i++) {
		if (contents[_undoneTaskIndexes[i]].getCode() == CODE_INVALID) {
...
std::string Undone::undo(void) {
	...
	for (unsigned int i = 0; i < undoneTasks.size(); i++) {
		...
		for (unsigned int j = 0; j < doneContents.size() && !isFoundDone; j++) {
			if (doneContents[j].getCode() == CODE_INVALID && doneContents[j].getDescription() == undoneTasks[i].getDescription() && doneContents[j].getStartDateTime() == undoneTasks[i].getStartDateTime() && doneContents[j].getEndDateTime() == undoneTasks[i].getEndDateTime() && doneContents[j].getLocation() == undoneTasks[i].getLocation()) {
		...
		for (unsigned int k = 0; k < todoContents.size() && !isFoundTodo; k++) {
			if (todoContents[k].getCode() != CODE_INVALID && todoContents[k].getDescription() == undoneTasks[i].getDescription() && todoContents[k].getStartDateTime() == undoneTasks[i].getStartDateTime() && todoContents[k].getEndDateTime() == undoneTasks[i].getEndDateTime() && todoContents[k].getLocation() == undoneTasks[i].getLocation()) {
...
std::string Undone::redo(void) {
	...
	for (unsigned int i = 0; i < undoneTasks.size(); i++) {
		...
		for (unsigned int j = 0; j < todoContents.size() && !isFoundTodo; j++) {
			if (todoContents[j].getCode() == CODE_INVALID && todoContents[j].getDescription() == undoneTasks[i].getDescription() && todoContents[j].getStartDateTime() == undoneTasks[i].getStartDateTime() && todoContents[j].getEndDateTime() == undoneTasks[i].getEndDateTime() && todoContents[j].getLocation() == undoneTasks[i].getLocation()) {
		...
		for (unsigned int k = 0; k < doneContents.size() && !isFoundDone; k++) {
			if (doneContents[k].getCode() != CODE_INVALID && doneContents[k].getDescription() == undoneTasks[i].getDescription() && doneContents[k].getStartDateTime() == undoneTasks[i].getStartDateTime() && doneContents[k].getEndDateTime() == undoneTasks[i].getEndDateTime() && doneContents[k].getLocation() == undoneTasks[i].getLocation()) {

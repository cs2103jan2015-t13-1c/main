...
//Date/time lengths, positions and month codes int:
const int LENGTH_MDAYSTRING = 2;
const int LENGTH_MONTHSTRING = 2;
const int LENGTH_YEARSTRING = 2;
const int LENGTH_HOURSTRING = 2;
const int LENGTH_MINSTRING = 2;
const int LENGTH_DATESTRING = LENGTH_MDAYSTRING + 1 + LENGTH_MONTHSTRING + 1 + LENGTH_YEARSTRING + 1 + LENGTH_HOURSTRING + LENGTH_MINSTRING;
const int POSITION_DATESEPARATOR1 = LENGTH_MDAYSTRING;
const int POSITION_DATESEPARATOR2 = LENGTH_MDAYSTRING + 1 + LENGTH_MONTHSTRING;
const int POSITION_DATETIMESEPARATOR = LENGTH_MDAYSTRING + 1 + LENGTH_MONTHSTRING + 1 + LENGTH_YEARSTRING;
...
//Empty attribute symbol string:
const std::string SYMBOL_EMPTYATTRIBUTE = "-";
...
void Task::initialise(std::string taskString) {
	...
	if (_code == CODE_INVALID) {
		isCorrect = true;
	}
	else if (_startDateTime > 0 && _endDateTime > 0) {
		isCorrect = (_code == CODE_TIMED);
	}
	else if (_startDateTime > 0 && _endDateTime == 0) {
		isCorrect = (_code == CODE_PARTIALTIMED);
	}
	else if (_endDateTime > 0 && _startDateTime == 0) {
		isCorrect = (_code == CODE_DEADLINE);
	}
	else {
		isCorrect = (_code == CODE_FLOATING);
	}
...
bool Task::checkTaskCodeValidity(std::string taskCode) {
	...
	if (taskCode == CODE_INVALID || taskCode == CODE_FLOATING || taskCode == CODE_TIMED || taskCode == CODE_PARTIALTIMED || taskCode == CODE_DEADLINE) {
		isValid = true;
	}
...
time_t Task::getTimeT(std::string dateTimeString) {
	...
	if (dateTimeString == "-") {
		...
		if (dateTimeString.size() != LENGTH_DATESTRING || dateTimeString[POSITION_DATESEPARATOR1] != '/' || dateTimeString[POSITION_DATESEPARATOR2] != '/' || dateTimeString[POSITION_DATETIMESEPARATOR] != ' ') {
		...
		if (dateTimeString.substr(0, LENGTH_MDAYSTRING).find_first_not_of(INTS) != std::string::npos) {
			throw exceptionTaskInvalidDate;
		}

		if (dateTimeString.substr(POSITION_DATESEPARATOR1 + 1, LENGTH_MONTHSTRING).find_first_not_of(INTS) != std::string::npos) {
			throw exceptionTaskInvalidDate;
		}

		if (dateTimeString.substr(POSITION_DATESEPARATOR2 + 1, LENGTH_YEARSTRING).find_first_not_of(INTS) != std::string::npos) {
			throw exceptionTaskInvalidDate;
		}

		if (dateTimeString.substr(POSITION_DATETIMESEPARATOR + 1, LENGTH_HOURSTRING).find_first_not_of(INTS) != std::string::npos) {
		...
		mday = atoi(dateTimeString.substr(0, LENGTH_MDAYSTRING).c_str());
		month = atoi(dateTimeString.substr(POSITION_DATESEPARATOR1 + 1, LENGTH_MONTHSTRING).c_str());
		year = atoi(dateTimeString.substr(POSITION_DATESEPARATOR2 + 1, LENGTH_YEARSTRING).c_str());
		hour = atoi(dateTimeString.substr(POSITION_DATETIMESEPARATOR + 1, LENGTH_HOURSTRING).c_str());
		min = atoi(dateTimeString.substr(POSITION_DATETIMESEPARATOR + 1 + LENGTH_HOURSTRING).c_str());

		...
		if (!keywordsAndParams[i].empty()) {
			...
			case INDEX_ENDDATETIME:
			{
				...
				if (isStartDateDefined && keywordsAndParams[i][0].size() == LENGTH_TIMEINPUT) {
					...
					if (startDateTimeString.substr(startDateTimeString.size() - LENGTH_TIMEINPUT).find_first_not_of(INTS) == std::string::npos) {
...
std::string Add::executeInput(std::string userInput) {
	...
	for (unsigned int i = 0; i < contents.size(); i++) {
		if (contents[i].getCode() != CODE_INVALID && contents[i].getDescription() == _description && contents[i].getStartDateTime() == _startDateTime && contents[i].getEndDateTime() == _endDateTime && contents[i].getLocation() == _location) {
